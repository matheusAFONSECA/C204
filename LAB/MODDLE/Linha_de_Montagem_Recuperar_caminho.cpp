...
int main()
{
	...
	// Entrada do número de máquinas por estação
	cin >> n;
	...
	// Caminho contrario 
	int k = n;  // estção da linha
	int linha_anterior = l_;  // linha anterior do caminho 
	cout << "Linha: " << l_ << " estacao " << n - 1 << endl;
	for(int k = n - 1; k > 0; k--){
		linha_anterior = l[linha_anterior][k];
		cout << "Linha: " << linha_anterior << " estacao " << k-1 << endl;
	}
